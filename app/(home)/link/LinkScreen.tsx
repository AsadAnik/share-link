...
// Demo links to show initially if no data is fetched
const demoLinks: LinkData[] = [
  { id: '1', platform: "GitHub", url: "https://www.github.com/benwright", isValid: true },
  { id: '2', platform: "YouTube", url: "https://www.youtube.com/benwright", isValid: true },
  { id: '3', platform: "LinkedIn", url: "https://www.linkedin.com/in/benwright", isValid: true },
];
...
const platformsConfig: { [key: string]: { name: string; regex: RegExp } } = {
  GitHub: {
    name: "GitHub",
    regex: /^https:\/\/(www\.)?github\.com\/(?!.*\.git$)[A-Za-z0-9_-]+(\/.*)?$/i,
  },
  LinkedIn: {
    name: "LinkedIn",
    regex: /^https:\/\/(www\.)?linkedin\.com\/(in|pub|company|school)\/[A-Za-z0-9_-]+\/?$/i,
  },
  YouTube: {
    name: "YouTube",
    regex: /^https:\/\/(www\.)?youtube\.com\/(@[A-Za-z0-9_-]+|c\/[A-Za-z0-9_-]+|channel\/[A-Za-z0-9_-]+|user\/[A-Za-z0-9_-]+|watch\?v=[\w-]+)$/i,
  },
};
...
const LinksPage = () => {
  ...
  // Sync fetched links data with local state
  useEffect(() => {
    if (linksData && linksData.length > 0) {
  ...
  const addLink = () => {
    setLinks([...links, { id: (links.length + 1).toString(), platform: "", url: "", isValid: true }]);
  };

  const updateLink = (id: string, platform: string, url: string) => {
    const isValid = validateLink(platform, url);
    setLinks(links.map(link => (link.id === id ? { ...link, platform, url, isValid } : link)));
  };
  ...
  // Function to validate URL based on platform
  ...
  // Handle saving links to the server
  const handleSaveLinkToServer = async () => {
    try {
      await createLinkMutation(links).unwrap();
      // After saving, revalidate the links to ensure they are correctly marked as valid
                    ...
                    >
                      ...
                      <div>
                        ...
                        <input
                          ...
                          value={link.url}
                          onChange={(e) => updateLink(link.id, link.platform, e.target.value)}
                          className={`w-full border ${link.isValid ? 'border-gray-300' : 'border-red-500'} rounded-lg px-3 py-2 focus:outline-none focus:ring focus:ring-blue-300`}
                          placeholder="https://example.com"
                        />
                        {!link.isValid && (
                          <p className="text-red-500 text-xs mt-1">Invalid URL for {platformsConfig[link.platform]?.name}</p>
                        )}
                      </div>
                    </div>
                  )}
